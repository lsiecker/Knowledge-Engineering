...
#     'start_activity': 'P2031',
#     'end_activity': 'P2032',
#     'instance_of': 'P31' #fuzzywuzzy addition
# }
...
#         SERVICE wikibase:label { bd:serviceParam wikibase:language "[AUTO_LANGUAGE],en". }
#     }
#     ''' % (matched_person_name, properties['birth_date'], properties['death_date'], properties['occupation'], properties['start_activity'], properties['end_activity'])
...
# # Define a function to query IMDb for a person's information
...
# # Iterate over the dataframe rows and query Wikidata for each person
# for index, row in df.iterrows():

#     person_name = row['person_name']

#     # Query the person information
#     results = query_person_info_WIKI(person_name)

#     # Extract the relevant information and store it in a dictionary
#     person_info = {
#         'person_name': person_name,
#         'birth_date': results[0]['birthdate']['value'] if results else '',
#         'death_date': results[0]['deathdate']['value'] if results and 'deathdate' in results[0] else '',
#         'start_activity': results[0]['start_activity']['value'] if results else '',
#         'end_activity': results[0]['end_activity']['value'] if results and 'end_activity' in results[0] else ''
#     }
#     # Append the person information to the list
...
# # Define a function to query IMDb for a person's information
# def query_person_info_IMDb(person_name):
#     # Search for the person by name
#     search_results = ia.search_person(person_name)
...
# # Find the duplicate instances in the DataFrame
# duplicates = df[df.duplicated(keep=False)]

# # Count the occurrences of each duplicate row
# duplicate_counts = duplicates['person_name'].value_counts()

# # Print the count and names of the duplicate instances
